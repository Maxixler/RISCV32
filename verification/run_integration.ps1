...
# Workaround for Vivado path encoding issues with Turkish dirs
#===========================================================

param(
    [string]$TestName = "test_recursive"   # Which test to run (default: test_recursive)
)
...
if (Test-Path $uart_log) {
    ...
    if ($content -match "ALL TESTS PASSED") {
        ...
        Write-Host "  >>> ${TestName}: FAIL <<<"
        Write-Host ""
        Write-Host "  UART Output:"
        Write-Host $content
        exit 1
    } else {
        Write-Host "  >>> ${TestName}: INCOMPLETE <<<"
        Write-Host ""
