...
(
    ...
    input  wire [ 5:0] EX_ALU_FUNC,  // ALU Function in EX Stage
...
// Carry-less multiply high-part logic
function [31:0] clmulh(input [31:0] x, input [31:0] y);
    reg [63:0] product;
    integer i;
    begin
        product = 64'h0;
...
// Carry-less multiply right-part logic
function [31:0] clmulr(input [31:0] x, input [31:0] y);
    reg [63:0] product;
    integer i;
    begin
        product = 64'h0;
...
function [31:0] clz(input [31:0] x);
    integer i;
    begin
        clz = 0;
...
// Count population (number of 1s) logic
function [31:0] cpop(input [31:0] x);
    integer i;
    begin
        cpop = 0;
        for (i = 0; i < 32; i = i + 1) begin
...
// Count trailing zeros logic
function [31:0] ctz(input [31:0] x);
    integer i;
    begin
        ctz = 0;
        for (i = 0; i < 32; i = i + 1) begin
...
// OR-combine bytes logic
function [31:0] orc_b(input [31:0] x);
    integer i;
    begin
        orc_b = 0;
...
// Reverse bytes logic
function [31:0] rev8(input [31:0] x);
    begin
        rev8 = {x[7:0], x[15:8], x[23:16], x[31:24]};
    end
endfunction

// Rotate left logic
function [31:0] rol(input [31:0] x, input [4:0] y);
    begin
        rol = (x << y) | (x >> (32 - y));
    end
endfunction
...
// Rotate right logic
function [31:0] ror(input [31:0] x, input [4:0] y);
    begin
        ror = (x >> y) | (x << (32 - y));
    end
endfunction
...
begin
    casez(EX_ALU_FUNC)
        ...
        `ALUFUNC_ADD_UW  : aluout1 = {1'b0, ex_busX} + {1'b0, ex_busY}; // Add with unsigned widening
        `ALUFUNC_ANDN    : aluout1 = ex_busX & ~ex_busY; // AND with negation
        `ALUFUNC_BCLR    : aluout1 = ex_busX & ~(1 << ex_busY[4:0]); // Bit clear
        `ALUFUNC_BCLRI   : aluout1 = ex_busX & ~(1 << EX_ALU_SHAMT[4:0]); // Bit clear immediate
        `ALUFUNC_BEXT    : aluout1 = (ex_busX >> ex_busY[4:0]) & 1; // Bit extract
        `ALUFUNC_BEXTI   : aluout1 = (ex_busX >> EX_ALU_SHAMT[4:0]) & 1; // Bit extract immediate
        `ALUFUNC_BINV    : aluout1 = ex_busX ^ (1 << ex_busY[4:0]); // Bit invert
        `ALUFUNC_BINVI   : aluout1 = ex_busX ^ (1 << EX_ALU_SHAMT[4:0]); // Bit invert immediate
        `ALUFUNC_BSET    : aluout1 = ex_busX | (1 << ex_busY[4:0]); // Bit set
        `ALUFUNC_BSETI   : aluout1 = ex_busX | (1 << EX_ALU_SHAMT[4:0]); // Bit set immediate
        `ALUFUNC_CLMUL   : begin
            aluout1 = 0;
            for (i = 0; i < 32; i = i + 1) begin
                if (ex_busY[i])
                    aluout1 = aluout1 ^ (ex_busX << i);
            end
        end

        `ALUFUNC_CLMULH    : aluout1 = clmulh(ex_busX, ex_busY); // Carry-less multiply high-part
        `ALUFUNC_CLMULR    : aluout1 = clmulr(ex_busX, ex_busY); // Carry-less multiply right-part
        `ALUFUNC_CLZ       : aluout1 = clz(ex_busX); // Count leading zeros
        `ALUFUNC_CLZW      : aluout1 = clz(ex_busX[31:0]); // Count leading zeros word
        `ALUFUNC_CPOP      : aluout1 = cpop(ex_busX); // Count population
        `ALUFUNC_CPOPW     : aluout1 = cpop(ex_busX[31:0]); // Count population word
        `ALUFUNC_CTZ       : aluout1 = ctz(ex_busX); // Count trailing zeros
        `ALUFUNC_CTZW      : aluout1 = ctz(ex_busX[31:0]); // Count trailing zeros word
        `ALUFUNC_MAX       : aluout1 = (ex_busX > ex_busY) ? ex_busX : ex_busY; // Maximum
        `ALUFUNC_MAXU_      : aluout1 = ($unsigned(ex_busX) > $unsigned(ex_busY)) ? ex_busX : ex_busY; // Maximum unsigned
        `ALUFUNC_MIN       : aluout1 = (ex_busX < ex_busY) ? ex_busX : ex_busY; // Minimum
        `ALUFUNC_MINU_      : aluout1 = ($unsigned(ex_busX) < $unsigned(ex_busY)) ? ex_busX : ex_busY; // Minimum unsigned
        `ALUFUNC_ORC_B     : aluout1 = orc_b(ex_busX); // OR-combine bytes
        `ALUFUNC_ORN       : aluout1 = ex_busX | ~ex_busY; // OR with negation
        `ALUFUNC_REV8      : aluout1 = rev8(ex_busX); // Reverse bytes
        `ALUFUNC_ROL       : aluout1 = rol(ex_busX, ex_busY); // Rotate left
        `ALUFUNC_ROLW      : aluout1 = rol(ex_busX[31:0], ex_busY[4:0]); // Rotate left word
        `ALUFUNC_ROR       : aluout1 = ror(ex_busX, ex_busY); // Rotate right
        `ALUFUNC_RORI      : aluout1 = ror(ex_busX, EX_ALU_SHAMT[4:0]); // Rotate right immediate
        `ALUFUNC_RORIW     : aluout1 = ror(ex_busX[31:0], EX_ALU_SHAMT[4:0]); // Rotate right immediate word
        `ALUFUNC_RORW      : aluout1 = ror(ex_busX[31:0], ex_busY[4:0]); // Rotate right word
        `ALUFUNC_SEXT_B    : aluout1 = { {24{ex_busX[7]}}, ex_busX[7:0] }; // Sign extend byte
        `ALUFUNC_SEXT_H    : aluout1 = { {16{ex_busX[15]}}, ex_busX[15:0] }; // Sign extend halfword
        `ALUFUNC_SH1ADD    : aluout1 = (ex_busX << 1) + ex_busY; // Shift left 1 and add
        `ALUFUNC_SH1ADD_UW : aluout1 = ({1'b0, ex_busX} << 1) + {1'b0, ex_busY}; // Shift left 1 and add unsigned widening
        `ALUFUNC_SH2ADD    : aluout1 = (ex_busX << 2) + ex_busY; // Shift left 2 and add
        `ALUFUNC_SH2ADD_UW : aluout1 = ({1'b0, ex_busX} << 2) + {1'b0, ex_busY}; // Shift left 2 and add unsigned widening
        `ALUFUNC_SH3ADD    : aluout1 = (ex_busX << 3) + ex_busY; // Shift left 3 and add
        `ALUFUNC_SH3ADD_UW : aluout1 = ({1'b0, ex_busX} << 3) + {1'b0, ex_busY}; // Shift left 3 and add unsigned widening
		  `ALUFUNC_SLLI_UW   : aluout1 = {1'b0, ex_busX} << ex_busY[4:0]; // Shift left logical immediate with unsigned widening
        `ALUFUNC_XNOR      : aluout1 = ~(ex_busX ^ ex_busY); // XNOR
        `ALUFUNC_ZEXT_H    : aluout1 = {16'b0, ex_busX[15:0]}; // Zero extend halfword
